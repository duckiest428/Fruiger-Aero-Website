...
export function Window({ app }: { app: WindowApp }) {
  ...
  return (
    ...
    >
      ...
      <div 
        className="h-10 flex items-center justify-between px-3 cursor-grab active:cursor-grabbing border-b border-black/40 relative overflow-hidden"
        style={{ 
          background: 'rgba(180, 230, 255, 0.35)',
          backdropFilter: 'blur(20px) saturate(180%)',
          boxShadow: 'inset 0 1px 0 rgba(255, 255, 255, 0.8)',
          borderBottom: '1px solid rgba(0, 50, 80, 0.4)'
        }}
        onPointerDown={(e) => dragControls.start(e)}
      >
        {/* Titlebar Glossy Top Half (Sharp diagonal split) */}
        <div className="absolute top-0 left-0 right-0 h-1/2 bg-gradient-to-b from-white/80 to-white/20 pointer-events-none before:content-[''] before:absolute before:inset-0 before:bg-gradient-to-tr before:from-transparent before:via-white/40 before:to-transparent before:rotate-12 before:scale-150"></div>

        <div className="flex items-center gap-2 font-semibold text-[#0f172a] drop-shadow-[0_1px_1px_rgba(255,255,255,1)] text-sm z-10 relative">
        ...
        <div className="flex gap-1 z-10 relative">
          ...
          <button 
            className="w-11 h-5 rounded-[3px] ml-1 bg-gradient-to-b from-red-500 to-red-700 hover:from-red-400 hover:to-red-500 flex items-center justify-center border border-red-900 shadow-[inset_0_1px_2px_rgba(255,255,255,0.8),_0_0_8px_rgba(255,0,0,0.4)] hover:shadow-[inset_0_1px_2px_rgba(255,255,255,1),_0_0_15px_rgba(255,50,50,0.8)] transition-all relative overflow-hidden group"
            onClick={(e) => { e.stopPropagation(); closeWindow(app.id); }}
            onPointerDown={(e) => e.stopPropagation()}
          >
             <div className="absolute top-0 left-0 right-0 h-1/2 bg-gradient-to-b from-white/70 to-white/10"></div>
             <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-8 h-3 bg-red-300 rounded-t-full opacity-0 group-hover:opacity-100 blur-[3px] transition-opacity"></div>
             <X size={16} color="white" className="z-10 drop-shadow-[0_1px_2px_rgba(0,0,0,0.8)] group-active:scale-90" strokeWidth={2.5} />
          </button>
        </div>
      </div>
